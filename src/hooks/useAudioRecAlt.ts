...
export const useAudioRecorderAlt = (): AudioRecorderHook => {
  ...
  const setupRecorder = (recorder: MediaRecorder) => {
    ...
    const handleDataAvailable = (e: BlobEvent) => {
      ...
      console.log('status is:', stateRef.current.status);
      if (state.status === 'paused') return;
      console.log('status is:', state.status);
    ...
    const handleStop = () => {
      ...
      try {
        ...
        console.log('✅ Chunks received:', chunksRef.current);
  ...
  const startRecording = async (retryCount = 0): Promise<void> => {
    try {
      ...
      if (!MediaRecorder.isTypeSupported(mimeTypeRef.current!)) {
        ...
        throw new Error(`Mime type ${mimeTypeRef.current} is not supported`);
      }
  ...
  const togglePauseResume = async () => {
    console.log('togglePauseResume function called');
    try {
      if (state.status === 'paused') {
        ...
        // update the blob and the url
        // wait 300ms before updating the state
        await new Promise((resolve) => setTimeout(resolve, 1000));
        const audioBlob = new Blob(chunksRef.current, {
          type: mimeTypeRef.current!,
        });
  ...
  }, [_clearTimer, _cleanTracks, state.blobUrl]);
